...
import { Upload, Search, Trash2, Database } from "lucide-react";
  ...
  async function importar(file: File) {
    ...
    try {
      ...
      for (const r of rows) {
        ...
        if (!skusVistos.has(sku)) { produtos.push({ sku, descricao }); skusVistos.add(sku); }
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            {total.toLocaleString("pt-BR")} SKUs · {totalEans.toLocaleString("pt-BR")} EANs
          </p>
        </div>
        <div className="flex gap-2">
          <input ref={fileRef} type="file" accept=".xlsx,.xls,.csv" className="hidden"
            onChange={(e) => e.target.files?.[0] && importar(e.target.files[0])} />
          <Button onClick={() => fileRef.current?.click()} disabled={importando} className="gap-1.5">
            {importando
              ? <><div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" />{progresso || "Importando..."}</>
              : <><Upload className="h-4 w-4" /> Importar planilha</>}
          </Button>
          {total > 0 && (
            <Button variant="outline" onClick={excluirTudo} disabled={importando} className="gap-1.5">
      ...
      <div className="rounded-xl border border-border bg-card p-4 flex items-start gap-3">
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          Planilha esperada: colunas com SKU, descrição e códigos de barras (EAN). Reconhece automaticamente
          colunas como "Nº do item", "Descrição", "Cod.Barras Master/Inner/Embalagem". Reimportar atualiza registros existentes.
        </p>
      </div>
      ...
      <div className="flex gap-2">
        <Input
          ...
          className="flex-1"
        />
        <Button onClick={buscar} variant="secondary" className="gap-1.5">
      ...
      {resultados.length > 0 && (
        <div className="space-y-2">
          {resultados.map((p) => (
            <div key={p.sku} className="bg-card border border-border rounded-xl p-3.5">
              <div className="flex items-start justify-between gap-2">
                <p className="font-mono font-semibold text-sm">{p.sku}</p>
                <span className="text-[10px] text-muted-foreground shrink-0">{p.eans.length} EAN(s)</span>
              </div>
              <p className="text-sm text-foreground/80 mt-0.5">{p.descricao}</p>
              {p.eans.length > 0 && (
                <p className="text-[11px] text-muted-foreground font-mono mt-1.5 break-all leading-relaxed">
                  {p.eans.join(" · ")}
                </p>
              )}
            </div>
      ...
      {busca && resultados.length === 0 && (
        <p className="text-sm text-muted-foreground text-center py-4">Nenhum resultado para "{busca}".</p>
      )}
    </div>
  );
}
